...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "meetingplanner" because\n';
  ...
  $wnd.__gwt_activeModules['meetingplanner'] = {
    'moduleName' : 'meetingplanner',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/meetingplanner.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['meetingplanner'].superdevmode = true;

  var expectedSuffix = '/meetingplanner.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + meetingplanner + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:meetingplanner';
  var devModeSessionKey = '__gwtDevModeSession:meetingplanner';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/meetingplanner/meetingplanner.nocache.js';
  ...
  $wnd.__gwt_activeModules['meetingplanner'].canRedirect = true;
  $wnd.__gwt_activeModules['meetingplanner'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING meetingplanner';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/meetingplanner?user.agent=' + ua + '&_callback=' + callback;
